...
""", unsafe_allow_html=True)
    ...
    if selected_scenario == "Custom Configuration":
        st.sidebar.markdown("#### Custom 5C Weights")
    ...
    if 'borrower_id' in data.columns:
        ...
        with col1:
            ...
            if 'loan_amount' in borrower_data:
                st.write(f"**Requested Amount**: ${borrower_data['loan_amount']:,.0f}")
            if 'age' in borrower_data and pd.notnull(borrower_data['age']):
                st.write(f"**Age**: {borrower_data['age']:.0f}")
            if 'annual_revenue' in borrower_data and pd.notnull(borrower_data['annual_revenue']) and borrower_data['annual_revenue'] > 0:
                st.write(f"**Annual Revenue**: ${borrower_data['annual_revenue']:,.0f}")
        ...
        with st.expander("View Character Score Details", expanded=False):
            character_raw = scores['character_score'] / weights['character'] if weights['character'] > 0 else 0
        ...
        with st.expander("View Capacity Score Details", expanded=False):
            capacity_raw = scores['capacity_score'] / weights['capacity'] if weights['capacity'] > 0 else 0
        ...
        with st.expander("View Capital Score Details", expanded=False):
            capital_raw = scores['capital_score'] / weights['capital'] if weights['capital'] > 0 else 0
        ...
        with st.expander("View Collateral Score Details", expanded=False):
            collateral_raw = scores['collateral_score'] / weights['collateral'] if weights['collateral'] > 0 else 0
        ...
        with st.expander("View Conditions Score Details", expanded=False):
            conditions_raw = scores['conditions_score'] / weights['conditions'] if weights['conditions'] > 0 else 0
        ...
        if character_score >= 0.7:
            st.success("Strong character profile with good credit history and reputation.")
        elif character_score >= 0.5:
            st.warning("Moderate character assessment. Some areas may need attention.")
        else:
            st.error("Character concerns identified. Requires additional verification.")
        ...
        if capacity_score >= 0.7:
            st.success("Strong repayment capacity based on cash flow and financial ratios.")
        elif capacity_score >= 0.5:
            st.warning("Adequate capacity but monitor closely for changes.")
        else:
            st.error("Capacity concerns. Cash flow may not support requested loan amount.")
        ...
        if capital_score >= 0.7:
            ...
            st.error("Insufficient capital cushion. Consider requiring additional equity.")
        ...
        if collateral_score >= 0.7:
            ...
            st.error("Insufficient collateral. Additional security may be required.")
        ...
        st.error("Borrower ID column not found in dataset. Please check data structure.")
